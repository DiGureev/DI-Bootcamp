...
# The menu should be saved to a local file – the program should load the file when it begins and add the updated details to the file 
#before exiting.

# The program should be written in two files – one which will deal with the UI (user interface), eg. showing the user menu and getting 
#user input, etc.
# The second file will handle all other functionality such as adding/removing items from the menu, along with loading and saving
# the data to a JSON file.
# Separating the files is important – the file which deals with the UI functionality should not have any details about the 
#menu file itself (encapsulation).
...
# remove_item(name) – if the item is in the menu, this function should remove it from the menu (and again do not save the changes 
#to the file yet) and return True. If the item was not in the menu, return False. (Hint: use Python’s del operator).

# save_to_file() - When the manager is finished updating the menu this function should be called and it should save all the updates 
#and write them into the the restaurant_menu.json file (ie. the file which holds the menu).
import json
import os
...
class MenuManager:
    ...
    def __init__(self):
        ...
        with open(dir_path + r"/restaurant_menu.json", "r") as f:
            self.menu = json.load(f)
    
    def add_item(self, name, price):
        self.menu['items'].append({'name': name, 
                                   'price': price})
    ...
    def save_to_file(self):
        dir_path = os.path.dirname(os.path.realpath(__file__))
        with open(dir_path + r"/restaurant_menu.json", "w") as f:
...
# add_item_to_menu() - this will ask the user to input the item’s name and price. It will not interact with the menu itself, 
...
# When the user chooses to exit the program, first write the menu to the file and then print a message that states that the menu was 
